...
namespace AweShark {
    ...
    public partial class App : Application {
        ...
        internal static void OnShowNewView(object sender, ShowCreatedWebViewEventArgs e) {
            ...
            else if (e.IsWindowOpen) {
                // No specs or only non-standard specs were specified, but the event is still 
                // the result of 'window.open'. We will open a normal window but we will still wrap 
                // the new native child view, maintaining its relationship with the parent window.
                newWindow.NativeView = e.NewViewInstance;
                // Show the window.
                newWindow.Show();
                }
            else {
                // The event is not the result of 'window.open', therefore it's most probably
                // the result of a link with target='_blank'. We will not be wrapping the created view; 
                // we let the WebControl hosted in ChildWindow create its own underlying view. 
                // Setting Cancel to true tells the core to destroy the created child view.
                //
